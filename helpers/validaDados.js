...
const schemaCategoria = Joi.object({
  tipo: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "O tipo de categoria precisa ter no mínimo 3 letras.",
...
const schemaCliente = Joi.object({
  ...
  senha: Joi.string()
    .pattern(/^(?=.*\d)(?=.*[a-zA-Z])(?=.*\W)[\d\w\W]{8,}$/)
    .required()
    .messages({
      "string.pattern.base":
        "A senha deve deve conter 8 caracteres no mínimo, ter ao menos 1 caracter especial e 1 número.",
      "any.required": "A senha é obrigatória.",
    }),
});

// Validação para Editora
...
const schemaFuncionario = Joi.object({
  ...
  isAdmin: Joi.boolean().messages({
    "boolean.base":
      "O valor de isAdmin deve ser true (verdadeiro) ou false (falso).",
  }),
});
